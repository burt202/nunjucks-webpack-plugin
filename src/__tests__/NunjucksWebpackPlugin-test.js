...
const webpackConfigBase = require("./config/config-base")
...
test("should execute successfully when option `templates` is passed", (t) => {
  ...
  return pify(webpack)(webpackConfig).then((stats) => {
    ...
    return pify(fs.readFile)(path.join(tmpDirectory, path.basename(templateName, ".njk")))
      .then((data) => {
      ...
      .then(() => pify(fs.readFile)(path.join(tmpDirectory, path.basename(templateName1, ".njk"))))
      .then((data) => {
        const contents = data.toString()
...
test("should execute successfully when option `templates` with `context` are passed", (t) => {
  ...
  return pify(webpack)(webpackConfig).then((stats) => {
    ...
    return pify(fs.readFile)(path.join(tmpDirectory, path.basename(templateName, ".njk"))).then(
      (data) => {
        const contents = data.toString()
...
//         t.true(contents.trim() === '<script src="029f359d6e9653eafd07.js"></script>')
